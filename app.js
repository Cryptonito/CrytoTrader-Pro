...
// Función para obtener pares USDC de Binance (filtrado)
async function fetchUSDCpairs() {
  const response = await fetch("https://api.binance.com/api/v3/exchangeInfo");
  const data = await response.json();
  const usdcPairs = data.symbols.filter(s => s.quoteAsset === "USDC" && s.status === "TRADING")
...
async function updateChartAndIndicators() {
  ...
  // Señal de trading basada en RSI
...
// Señales trading basadas en RSI
...
function drawChart(prices, support, resistance){
  clearCanvas();
  ...
  // Soporte (línea amarilla)
  let supY = priceCanvas.height - ((support - minPrice) / (maxPrice - minPrice))*priceCanvas.height;
  ctx.beginPath();
  ...
  // Resistencia (línea roja)
  let resY = priceCanvas.height - ((resistance - minPrice) / (maxPrice - minPrice))*priceCanvas.height;
  ctx.beginPath();
  ctx.strokeStyle = 'red';
  ctx.moveTo(0,resY);
